...
import { Download, ExternalLink, Calendar, Music, Eye, Copy } from 'lucide-react';
import { SpotifyAlbum } from '../types/spotify';
import { useState } from 'react';

interface AlbumCardProps {
  album: SpotifyAlbum;
  onPreview?: (album: SpotifyAlbum) => void;
}

export default function AlbumCard({ album, onPreview }: AlbumCardProps) {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const filename = `${album.artists[0]?.name} - ${album.name}.jpg`
  ...
  const handleCopyUrl = async () => {
    try {
      await navigator.clipboard.writeText(album.external_urls.spotify);
      setCopied(true);
